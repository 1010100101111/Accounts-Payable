...
logger=logging.getLogger("Account Payables")
logger.setLevel(logging.DEBUG)
fh=logging.FileHandler("accpay.log")
...
class AmountError(Exception):
    pass
def filter(string):
    for syntax in [";","--","SELECT","DROP","ALTER","DELETE","CREATE","INSERT","UPDATE","RENAME","TABLE","COLUMN","moredetails"]:
        if any([syntax in string,syntax.lower() in string,syntax.upper() in string]):
            logger.error("Someone tried to do SQL injection to the database.")
            raise SQLInjectionError("SQL Query detected!")
...
def refresh():
    ...
    if not duedates==[]:
        for res in duedates:
            ...
            message = "'s due date is today" if duedate[0][0]==today else " has expired"
            tkinter.messagebox.showinfo("SYSTEM - Due date alert","Invoice no.{} from vendor {}{}. ".format(res[1],res[0],message))
...
def summary_csv():
    with open('summary.csv',mode='w') as file:
        writer = csv.writer(file,delimiter=",",quotechar='"',quoting=csv.QUOTE_MINIMAL)
        writer.writerow(['Vendor Name','Total Amount'])
        query.execute('SELECT DISTINCT Vendor FROM moredetails')
        vendor = query.fetchall()
        for i in range(len(vendor)):
            query.execute('SELECT SUM(InvoiceAmount) FROM moredetails WHERE Vendor="{}"'.format(vendor[i][0]))
            amount = query.fetchall()
            writer.writerow([vendor[i][0],'{:,}'.format(int(amount[0][0]))])
        tkinter.messagebox.showinfo("DONE","Successfully wrote data to file 'summary.csv'")
button01 = tkinter.Button(summary,text="Summary",command=summary_info)
button01.grid(column=0,row=1)
button02 = tkinter.Button(summary,text="Port to Excel",command=summary_csv)
button02.grid(column=0,row=2)
...
def details_csv():
    with open('details.csv',mode='w') as file:
        writer = csv.writer(file,delimiter=",",quotechar='"',quoting=csv.QUOTE_MINIMAL)
        writer.writerow(['Vendor Name','Invoice Number','Invoice Date','Invoice Amount','Due Date'])
        query.execute('SELECT * FROM moredetails')
        sqlfetch = query.fetchall()
        for i in range(len(sqlfetch)):
            writer.writerow(["'"+sqlfetch[i][0]+"'","'"+sqlfetch[i][1]+"'","'"+sqlfetch[i][2]+"'",'{:,}'.format(int(sqlfetch[i][3])),'None' if sqlfetch[i][4]==None else "'"+sqlfetch[i][4]+"'"])
        tkinter.messagebox.showinfo("DONE", "Successfully wrote data to file 'details.csv'")
button11 = tkinter.Button(details,text="Details",command=detailed_info)
button11.grid(column=0,row=1)
button12 = tkinter.Button(details,text="Port to Excel",command=details_csv)
...
def get():
    ...
    try:
        ...
        tkinter.messagebox.showerror("INVALID AMOUNT","Alphabets and special characters detected")
        return
    try:
        filter(vendor)
        filter(no)
        ...
        filter(amount)
        filter(duedate)
        year_input = int(strdate.split("-")[0])
        month_input = int(strdate.split("-")[1])
        date_input = int(strdate.split("-")[2])
        if year_input!=date.today().year:
            raise DateError("Invalid year")
        if int(amount)<=0:
            raise AmountError("Invalid amount!")
        today = date.today()
        if today.month<month_input:
            raise DateError("The invoice date is in the future.")
        if today.day<date_input and today.month==month_input:
            raise DateError("The invoice date is in the future.")
        if date_input>31:
            raise DateError("Wrong date")
        if date_input>30 and (month_input in [4,6,9,11]):
            raise DateError("Wrong date")
        if (month_input==2) and (year_input%4==0) and (date_input>29):
            raise DateError("Wrong date")
        if (month_input==2) and (year_input%4!=0) and (date_input>28):
            raise DateError("Wrong date")
    except SQLInjectionError as e:
        tkinter.messagebox.showerror("SQL INJECTION DETECTED!",e)
        return
    except DateError as e:
        tkinter.messagebox.showerror("INVALID DATE",e)
        return
    except AmountError as e:
        tkinter.messagebox.showerror("INVALID AMOUNT",e)
        return
    # format_date=r"\d{4}\-(0?[1-9]|1[012])\-(0?[1-9]|[12][0-9]|3[01])*"
    # if not re.match(format_date,strdate):
    #     tkinter.messagebox.showwarning("WRONG FORMAT","The format for date accepted is YYYY-MM-DD")
    #     return
    duedate = "NULL" if duedate=="" else '"'+duedate+'"'
    query.execute("SELECT DISTINCT InvoiceNO FROM moredetails;")
    invoices = query.fetchall()
    for i in invoices:
        if no==i[0]:
            tkinter.messagebox.showerror("INVALID INVOICE NUMBER","An invoice with same invoice number is found in the database.")
            return
    query.execute('INSERT INTO moredetails VALUES ("{}","{}","{}",{},{})'.format(vendor,no,strdate,amount,duedate))
    database.commit()
    refresh()
    logger.info("An invoice has been added to the database.")
...
combo0 = tkinter.ttk.Combobox(pay_frame,state="readonly")
combo0.grid(column=1,row=0)
value0 = []
for i in range(len(list0)):
    value0.append(list0[i])
combo0['values'] = tuple(value0)
combo1 = tkinter.ttk.Combobox(pay_frame,state="readonly")
...
def paid():
    ...
    if amount>int(total[0][0]):
        ...
        if tkinter.messagebox.askokcancel("Confirmation","Are you sure you want to pay this invoice?\n\nAmount = {:_}".format(int(total[0][0])).replace("_",",")):
            ...
            database.commit()
            refresh()
            addtext2.configure(text="PAY-Amount : ")
            logger.info(f"Fully paid invoice from {vendor} with invoice number {no}.")
        ...
        if tkinter.messagebox.askokcancel("Confirmation - PARTIAL PAYMENT","Are you sure you want to pay this invoice?\n\nAmount = {:_}\nPayment amount = {:_}\nRemaining invoice amount = {:_}".format(int(total[0][0]),amount,remains).replace("_",",")):
            ...
            refresh()
            addtext2.configure(text="PAY-Amount : ")
            logger.info(f"Partially paid {int(total[0][0])} to invoice from {vendor} with invoice number {no}, remaining amount = {remains}.")
...
# TODO - Make a report system from the logged data
window.mainloop()
